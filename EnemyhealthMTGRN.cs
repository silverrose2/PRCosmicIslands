...
public class EnemyhealthMTGRN : MonoBehaviour// note new method wait time you could update this to other standrard health would take time  but save haning to create new damage to give
{// had to create a new script here too duipcate reanme guess as it in use
    ...
    void Start()
    {
      ...
      //  Destroy(gameObject, 50);//time for zombies to vanish
    ...
    void Update()
    {
        ...
        if (timer > waitTime)
        {
          //  Destroy(gameObject); TURNED OFF BUT COULD DESTROY AFTER TIME
            timer = 0f;
        }
    }
    ...
    void OnTriggerEnter(Collider other)
    {
                                                ...
                                                // if (other.tag == "Player destroy" && !isDead) // if player is dead 'tag condition' the following function below happen
        {
            ...
            Destroy(GetComponent<Randommovement>());//stop random move
                                                    ...
                                                    //anim.SetInteger("Condition",5);// fallbackwards anim error this way fix on just anim play
           ...
           // ScorePR.scoreValue += 10;// ******************** score added // ignore this just showing what id done here to improve as line below
           ScoremtgrnPR.scoreValue += (10);// new method as now has its own dedicated script so doesnt conflict
        ...
        IEnumerator delay(int v) // remove this once level 2 in place
        {
            ...
            yield return new WaitForSeconds(1.6f);// seconds delay 6 about right
            anim.speed = 0;
            gameObject.GetComponent<UnityEngine.AI.NavMeshAgent>().enabled = false;
