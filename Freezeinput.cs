...
public class Freezeinput : MonoBehaviour// PR This scipt only works if you have the game object dragged
                                        ...
                                        // found in documents // https://opsive.com/support/documentation/ultimate-character-controller/input/you can adapt
{
    ...
    void Update()// replaced start // Dont freeze to soon or button disables before playing anim- fraction of a second
    {

        if (Input.GetButtonDown("Equip Next Item"))// string set up as y button/ joystick button 3
                                                   // You'll see a drop down for every button or key scroll look for 'joystick button 3'.
        {
            ...
            {//.4 to allow drift to full idle to avoid anim transition probs. works best from completed idle pose
                yield return new WaitForSeconds(0.4f);// allow fraction of time so Y button can be pressed 0.4 wait
                EventHandler.ExecuteEvent(Femalefighter2, "OnEnableGameplayInput", false);// freeze
                yield return new WaitForSeconds(0.50f);// higher the better less issues time to unfreeze this is the only one to adjust really was 2
