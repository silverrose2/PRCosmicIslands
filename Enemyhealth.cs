...
public class Enemyhealth : MonoBehaviour// *********** used on raptor
{
    ...
    void OnTriggerEnter(Collider other)
    {
                                                ...
                                                // if (other.tag == "Player destroy" && !isDead) // if player is dead 'tag condition' the following function below happen
        {
            ...
            Destroy(GetComponent<MovebeforenavPR>());   // myObject.GetComponent<BoxCollider>().enabled = false;
            Destroy(GetComponent<EnemyfollowNavtype2PR>());
            Destroy(GetComponent<EnemyfollowplayerOrigPR>());// all various scripts used in movement through the game destroyed here
            ...
            ScorePR.scoreValue += (10);// this must always relate to the score sript being used
            ScoreconstantPR.scoreValue += (10);// this must always relate to the score sript being used
