...
public class GenerateraptorMGLevel : MonoBehaviour

{
    ...
    IEnumerator EnemyDrop()
    {
        ...
        //**********************************************************update
        yield return new WaitForSeconds(5);//new delay for spawn pr// note 35 is last default cout changed to 15 7.4.23 0 immediate
        //55 last default count
        while (enemyCount < 21)//7//x3 spawns// 125, 250 default how many enemies you need 300 is really enough or too much after that you cant move
        {
            theEnemy.SetActive(true);//new delay for spawn // note plotting below simple use cube to take measurements
            yield return new WaitForSeconds(0.01f);
            xPos = Random.Range(-90, 500);//-40,50 //plot out a square where the spawn is try large area x 0,70 , z 0,-40 use a gameobject to measure//(-300, 400);
            zPos = Random.Range(-1000, -300);// -35,40""// default was -50,30 t      // (-160, -600)
